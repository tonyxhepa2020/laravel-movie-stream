<section>
    ...
    @include('includes.welcome-section')
    <div class="clearfix"></div>
    <div class="container mx-auto bg-gray-2000 rounded">
        <section class="my-3 px-4 py-2 mx-auto">
            <div class="flex justify-between rounded my-2 px-2 bg-blue-deep">
                <h2 class="text-xl text-center p-2">Filmat e Fundit</h2>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 rounded">
                @foreach($movies as $movie)
                    ...
                    <div class="relative" wire:loading.remove>
                        <a href="{{ route('movies.show', $movie->slug) }}">
                            <img
                                class="w-full h-full rounded hover:opacity-75 transition transition-900 transition-ease-in bg-blue-deep"
                   ...
                   <span class="py-1 bg-gray-900 text-white rounded text-xs font-bold px-1">
                       {{ $movie->video_format }}
                        ...
                        <div class="absolute flex flex-col left-0 top-0 my-2 mx-1">
                            ...
                            >{{ date('Y', strtotime($movie->release_date))}}</span>
                        </div>
                        <div class="inset-x-0 absolute flex bottom-0 bg-blue-deep bg-opacity-75 hover:bg-blue-300 hover:text-gray-900 p-2 font-bold rounded-b">
                            <a href="{{ route('movies.show', $movie->slug) }}" class="w-full text-sm"><h2>{{ $movie->title}}</h2></a>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="container mx-auto bg-gray-2000 rounded">
        <section class="my-3 px-4 py-2 mx-auto">
            <div class="flex justify-between rounded my-2 px-2 bg-blue-deep">
                <h2 class="text-xl text-center p-2">Serialet e Fundit</h2>
            </div>
